...
st.title("🌊 סימולטור זרימה מקסימלית בזמן אמת (Max Flow) דינמי")
st.write("מערכת ניהול ולוגיסטיקה דינמית: העלי קובץ אקסל, שני את עומס המטבח או השביתי רובוטים כדי לראות כיצד צוואר הבקבוק משתנה אוטומטית.")

st.divider()

# רכיב להעלאת קובץ האקסל של נתוני הקיבולת והזרימה
uploaded_flow = st.file_uploader("📊 העלי קובץ אקסל (xlsx) של נתוני קיבולות הרשת:", type=["xlsx"])
...
if uploaded_flow is not None:
    try:
        ...
        if df.shape[1] >= 3:
            ...
            st.success(f"✅ נתוני קיבולות הרשת נטענו בהצלחה! זוהו {len(DYNAMIC_ROBOTS)} אזורי הפצה שונים ברשת.")
            ...
            st.write("🤖 **סטטוס תפעולי של צי הרובוטים (סמני להשבתת רובוט):**")
            robots_status = {}
            ...
            for idx, zone in enumerate(DYNAMIC_ROBOTS.keys()):
                with cols[idx]:
                    # חילוץ המילה הראשונה בשם האזור כדי שהכפתור יהיה קצר ויפה
            ...
            for zone, caps in DYNAMIC_ROBOTS.items():
                ...
                # בדיקת צוואר בקבוק מקומית באזור פעיל: אם קיבולת השולחנות חנקה את הרובוט
                if t_cap < r_cap and not robots_status[zone]:
                    bottlenecks.append(f"**{zone}** (קיבולת שולחנות: {t_cap} מול קיבולת רובוט: {r_cap})")
                # אם הרובוט עצמו הושבת, הוא הופך לצוואר בקבוק קריטי
                elif robots_status[zone]:
                    bottlenecks.append(f"**{zone}** (הרובוט הושבת עקב תקלה תפעולית / יצא מצי השירות)")
                
                # הצגת המצב התפעולי של האזור
                st.info(f"""
            ...
            # אילוץ עליון של הרשת: הזרימה הכוללת לא יכולה לעבור את מה שהמטבח מייצר בפועל
            final_flow = min(kitchen_cap, total_max_flow)
            
            st.divider()
            st.subheader("🎯 סיכום תוצאות המודל")
            
            # הצגת מטריקות סופיות דינמיות
            ...
            with res_col3:
                st.metric(label="📊 נצילות פוטנציאל המטבח", value=f"{int((final_flow / kitchen_cap) * 100)}%")
            ...
            if final_flow == kitchen_cap and final_flow < total_max_flow:
                ...
                for b in bottlenecks:
                    st.write(f"* {b}")
                st.caption("💡 הסבר אלגוריתמי: באזורים אלו, הזרימה נחסמה ונעצרה על ערך המינימום – או בגלל שקיבולת ספיגת השולחנות נמוכה מכושר ההובלה של הרובוט, או בשל השבתה תפעולית מוחלטת של הרובוט באותו אזור.")
            else:
    ...
    st.info("💡 אנא העלי קובץ אקסל (xlsx) המכיל את נתוני הקיבולת של אזורי השירות כדי להפעיל את סימולציית ה-Max Flow.")
